...
#' @param crown_shp A polygon shapefile that overlays r
...
#' @export
assign_crowncells_quantile <- function(r, crown_shp, outpname_r, plott = F, quantiles_to_keep = NULL, outpname_shp){
  ...
  outp <- raster::rasterize(x = outp_sp, y = r, field = colnames(outp_i_sp@data)[1], fun = function(x,...){max(x)},
  ...
  if (!is.null(quantiles_to_keep) & !is.null(outpname_shp)){
   sapply(slot(crowns_trimmed, "polygons"), function(x) slot(x, "ID"))
    crowns_outp <- sp::SpatialPolygonsDataFrame(crowns_trimmed, data = crowns@data)
