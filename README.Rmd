...
* write a shadow-seaker function, given a maximum tree height, sun azimuth
if there is no pixel with low PAN value in the projected shadow, your seed might not be a tree top (unless there is another
seed in the projected crown)

* if Ganguly's the tree non-tree mask is good, than the shadow-seaker might not be neScessary...
...
## Crown delineation - region_growing_wrapper.r
...
## Mask the shadow sections of a crown - assigns_crown_cells_quantile

#### Challenge

Portions of crowns can be quite heavily shaded, comprising the spectral indices from the. Furthermore, when crowns delineated in 
...
This value (e.g. NIR reflectance rank in the crown) can then be used as a mask while extract thermal metrics from crowns.
...
#### Output
A raster file, with the same spatial attributes as r and values between 0 and 100 for cells in polygons, and NA outside polygons.
...
where layers do the same, as long as the columns/layers are named using the Quantalab convention *X700.000000.Nanometers*
...
he attributes in the output have a 'i#' prefex referring
